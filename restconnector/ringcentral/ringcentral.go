...
import (
	...
	"github.com/grokify/go-saviynt/restconnector"
)

func ObjectSummary() restconnector.ObjectSummary {
	return restconnector.ObjectSummary{
		Objects: restconnector.Objects{
			Connection: []string{"RingCentral"},
		},
		Information: restconnector.ObjectSummaryInformation{
...
func Connection() restconnector.ConnectionFile {
	ra := restconnector.RequestsAttribute{
		Name: "CreateAccountJSON",
		Requests: restconnector.Requests{
			AccountIDPath: "call1.message.id",
			ResponseColsToPropsMap: map[string]string{
				"name": "call1.message.userName~#~char",
			},
			Calls: []restconnector.Call{
				{
					Name:       "call1",
					Connection: "acctAuth",
					...
					SuccessResponses: restconnector.Responses{
	...
	extAttrs := restconnector.ExternalAttrs{
		{
	...
	return restconnector.ConnectionFile{
		"RingCentral": restconnector.Connection{
